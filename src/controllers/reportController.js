...
exports.generateEvidenceReport = async (req, res) => {
    ...
    const { caseId, evidenceIds, content } = req.body;

    // const effectiveCaseId = caseIdFromParams || caseId; // Escolhe de onde vem o caseId

    try {
        // Validações
        if (!caseId || !mongoose.Types.ObjectId.isValid(caseId)) { // Usando caseId do body aqui
            return res.status(400).json({ error: "ID de caso inválido ou ausente no corpo da requisição." });
        }
        if (!Array.isArray(evidenceIds) || evidenceIds.length === 0) {
            return res.status(400).json({ error: "É necessário fornecer um array de IDs de evidências." });
        }
        for (const evId of evidenceIds) {
            if (!mongoose.Types.ObjectId.isValid(evId)) {
                return res.status(400).json({ error: `ID de evidência inválido encontrado: ${evId}` });
            }
        }
        if (!content) {
        ...
        // Busca dados do Caso
        const caso = await Case.findById(caseId)
            .populate('responsibleExpert', 'name role'); // Popula o responsável para checagem de permissão e info no laudo

        // Busca as Evidências específicas
        const evidencesForReport = await Evidence.find({
            '_id': { $in: evidenceIds }, // Busca todas as evidências cujos IDs estão no array
            'caseId': caseId // Garante que as evidências pertencem ao caso especificado
        }).populate('collectedBy', 'name role');

        const userGeneratingReport = await User.findById(performingUserId);

        if (!caso) return res.status(404).json({ error: "Caso não encontrado." });
        if (!userGeneratingReport) return res.status(404).json({ error: "Usuário assinante não encontrado." });
        if (evidencesForReport.length !== evidenceIds.length) {
            // Alguma(s) evidência(s) não foi/foram encontrada(s) ou não pertencem ao caso.
            return res.status(404).json({ error: "Uma ou mais evidências não foram encontradas ou não pertencem ao caso especificado." });
        }

        // --- LÓGICA DE PERMISSÃO (similar à generateReport, mas pode ser ajustada) ---
        const isAdmin = performingUserRole === 'admin';
        const isResponsibleExpert = caso.responsibleExpert && caso.responsibleExpert._id.toString() === performingUserId.toString();
        let isPeritoTeamMember = false;
        if (caso.team && Array.isArray(caso.team)) { // Supondo que 'team' está no objeto 'caso' (se não, popule)
            // Se 'team' não for populado em 'caso', você precisaria buscar o usuário do time ou verificar a role de outra forma
            const teamMemberDetails = await User.findById(performingUserId).select('role'); // Busca a role se não tiver no token
            if (teamMemberDetails && teamMemberDetails.role === 'perito' || teamMemberDetails.role === 'assistente' && caso.team.some(memberId => memberId.toString() === performingUserId.toString())) {
                isPeritoTeamMember = true;
            }
        }
        if (!(isAdmin || isResponsibleExpert || isPeritoTeamMember)) {
            return res.status(403).json({ error: "Acesso negado. Apenas ADM, o Perito Responsável ou estar na equipe podem gerar laudos." });
        }
        // --- FIM DA LÓGICA DE PERMISSÃO ---

        // ---- CONSTRUÇÃO DO DOCUMENTO PDF PARA EVIDÊNCIAS ESPECÍFICAS ----
        const documentContent = [
            { text: 'LAUDO PERICIAL DE EVIDÊNCIA(S)', style: 'header' },
            { text: `Referente ao Caso nº: ${caso._id.toString()}`, alignment: 'center', margin: [0, 0, 0, 5], fontSize: 10 },
            { text: `Nome do Caso: ${caso.nameCase || 'Não informado'}`, alignment: 'center', margin: [0, 0, 0, 15], fontSize: 14, bold: true },
            // ... (outras informações do caso que você queira incluir, como em generateReport) ...
            { text: `Emitido em: ${new Date().toLocaleDateString('pt-BR')} às ${new Date().toLocaleTimeString('pt-BR')}`, alignment: 'right', margin: [0, 0, 0, 20], fontSize: 10 },
            { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1, lineColor: '#cccccc' }], margin: [0, 0, 0, 20] },


            { text: '1. EVIDÊNCIA(S) ANALISADA(S)', style: 'subheader' },
        ];

        if (evidencesForReport.length > 0) {
            evidencesForReport.forEach((ev, index) => {
                documentContent.push({ text: `Evidência ${index + 1}: ${ev.title || 'Sem Título'} (ID: ${ev._id})`, style: 'evidenceTitle' });
                // ... (código para adicionar detalhes da evidência: tipo, descrição, data, coletado por, etc.)
                // Similar ao que você faz em generateReport, mas focado apenas nas evidências em 'evidencesForReport'
                documentContent.push({
                    columns: [
                        { width: 'auto', text: [{ text: 'Tipo:', bold: true }, ` ${ev.evidenceType || 'Desconhecido'}`] },
                        { width: '*', text: [{ text: 'Coletado por:', bold: true }, ` ${ev.collectedBy?.name || 'Não informado'} (${ev.collectedBy?.role || ''})`] }
                ...
                if (index < evidencesForReport.length - 1) { documentContent.push({ text: '', margin: [0, 0, 0, 10] }); documentContent.push({ canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 0.5, lineColor: '#eeeeee' }], margin: [0, 0, 0, 10] }); }
            });
        } else {
            // Essa condição não deveria ser atingida se a validação acima (evidencesForReport.length !== evidenceIds.length) estiver correta
            documentContent.push({ text: 'Nenhuma evidência selecionada ou encontrada para este laudo.', margin: [0, 10, 0, 0], italics: true });
        }


        documentContent.push({
            text: '2. CONSIDERAÇÕES PERICIAIS / ANÁLISE DAS EVIDÊNCIAS',
            style: 'subheader'
        });

        documentContent.push({
            text: content || 'Nenhum conteúdo de laudo fornecido.',
            margin: [0, 5, 0, 20]
        });

        // Seção de assinatura (similar a generateReport)
        documentContent.push({ text: '', pageBreak: evidencesForReport.length > 2 ? 'before' : undefined, margin: [0, 40, 0, 0] }); // Ajuste pageBreak conforme necessário
        documentContent.push({ text: 'Assinatura do Perito Responsável pelo Laudo', style: 'subheader', alignment: 'center' });
        documentContent.push({ text: '\n\n\n________________________________________', alignment: 'center', margin: [0, 20, 0, 5] });
        documentContent.push({ text: `${userGeneratingReport.name || 'Nome não informado'} (${userGeneratingReport.role || 'Função não definida'})`, alignment: 'center' });


        const pdfStyles = { /* ... seus estilos ... */ };
        const docDefinition = {
            content: documentContent,
            styles: pdfStyles,
            defaultStyle: { font: 'Roboto', fontSize: 11, lineHeight: 1.3 },
        ...
        const filename = `laudo-evidencias-caso-${caso.nameCase.replace(/[^a-zA-Z0-9\s-]/g, '').replace(/\s+/g, '_')}-${Date.now()}.pdf`;
        const pdfUrl = await salvarPDFNoStorage(pdfBuffer, filename, req);

        // Salvar o registro do Laudo no banco
        // Considere adicionar um campo 'type' ou 'scope' ao modelo Report para diferenciar laudos de caso e laudos de evidência
        // ou um campo 'relatedEvidences' que seria um array de ObjectIds.
        const newReport = new Report({
            caseId: caso._id,
            relatedEvidences: evidenceIds, // Novo campo para armazenar os IDs das evidências deste laudo
            reportType: 'evidence_specific', // Novo campo para tipo de laudo
            content,
            signedBy: userGeneratingReport._id,
            pdfUrl,
            status: 'finalizado',
        });
        await newReport.save();
        ...
        saveAuditLog(performingUserId, 'CREATE_EVIDENCE_REPORT', 'Report', newReportId, { caseId: caso._id, evidenceIds, pdfUrl });

        res.status(201).json({
            message: "Laudo de evidência(s) gerado com sucesso!",
            reportId: newReportId,
            pdfUrl,
        });

    } catch (err) {
        console.error('Erro no generateEvidenceReport:', err);
        saveAuditLog(performingUserId, 'CREATE_EVIDENCE_REPORT_FAILED', 'Report', newReportId || caseId, { caseId, evidenceIds, error: err.message });
        res.status(500).json({
            error: "Erro interno do servidor ao gerar o laudo de evidência(s).",
            details: process.env.NODE_ENV === 'development' ? err.message : undefined,
        });
    }
};